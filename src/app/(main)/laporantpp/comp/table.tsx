                ...
                <table className="w-full">
                    <thead>
                        <tr className={`text-white ${data.is_sekretariat ? "bg-emerald-500" : "bg-blue-500"}`}>
                            ...
                            <th className="border-r border-b py-3 px-4 border-gray-300 min-w-[200px] text-center">POT BPJS 4</th>
                    ...
                    <tbody>
                        {data?.penilaian_kinerjas ?
                            data?.penilaian_kinerjas
                                .map((item: PenilaianGroupedResponse, index: number) => (
                                    <tr key={index}>
                                        ...
                                        <td className={`border ${data.is_sekretariat ? "border-emerald-500" : "border-blue-500"} px-6 py-4 text-center`}>{`${percentDisplay(item.kinerja_kehadiran)}%` || 0}</td>
                                        ...
                                        <td className={`border ${data.is_sekretariat ? "border-emerald-500" : "border-blue-500"} px-6 py-4 text-center`}>Rp.{formatRupiah(item.tpp_pegawai?.bpjs_1) ?? 0}</td>
                                        <td className={`border ${data.is_sekretariat ? "border-emerald-500" : "border-blue-500"} px-6 py-4 text-center`}>Rp.{formatRupiah(item.tpp_pegawai?.bpjs_4) ?? 0}</td>
                            ...
                            :
                            <tr>
                                <td colSpan={19} className={`border ${data.is_sekretariat ? "border-emerald-500" : "border-blue-500"} px-6 py-4`}>Data Anggota Kosong</td>
                            </tr>
                        }
                    </tbody>
                    <tfoot>
                        <tr>
                            <td colSpan={17} className={`border ${data.is_sekretariat ? "border-emerald-500" : "border-blue-500"} px-6 py-4`}>Total</td>
                            <td className={`border ${data.is_sekretariat ? "border-emerald-500" : "border-blue-500"} px-6 py-4`}>
                                Rp.{formatRupiah(totalJumlahBersih(data?.penilaian_kinerjas))}
                            </td>
                            <td className={`border ${data.is_sekretariat ? "border-emerald-500" : "border-blue-500"} px-6 py-4`}></td>
                        </tr>
                    </tfoot>
                </table>
            </TableComponent>
...
function totalJumlahBersih(items?: PenilaianGroupedResponse[]) {
    if (!items) return 0;

    return items.reduce((total, item) => {
        const nilai = item.tpp_pegawai?.jumlah_bersih ?? 0;
        return total + Number(nilai);
    }, 0);
}
