...
export const Table: React.FC<Table> = ({ data }) => {
  ...
  const url = useMemo(() => {
    ...
    return `/api-laporan/timkerja_sekretariat/${data.kode_tim}/rencana_kinerja?tahun=${tahun}&bulan=${bulan}`;
  }, [isReady, tahun, bulan]);
  ...
  const hapusRekin = async (id: number) => {
    await apiFetch(`/api-laporan/timkerja_sekretariat/${data.kode_tim}/rencana_kinerja/${id}`, {
      method: "DELETE",
    }).then(_resp => {
