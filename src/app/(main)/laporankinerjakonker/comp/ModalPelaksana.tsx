...
export const ModalPelaksana: React.FC<Modal> = ({ isOpen, onClose, onSuccess, id_program, Data }) => {
    ...
    const { data: DataTim, error, loading } = useGet<TimGetResponse[]>(`/api-laporan/timkerja/timkerja?tahun=${tahun}&bulan=${bulan}`)
    ...
    const onSubmit: SubmitHandler<FormValue> = async (data) => {
        ...
        if(data.pegawai_id?.value === undefined || data.pegawai_id?.value === null){
            ...
            try {
                setProses(true);
                await apiFetch(`/api-laporan/timkerja/petugas_tim`, {
                    method: "POST",
                    body: payload as any
                }).then(_ => {
