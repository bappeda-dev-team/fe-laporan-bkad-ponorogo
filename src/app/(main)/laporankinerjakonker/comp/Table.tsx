...
export const Table = () => {
  ...
  const url = useMemo(() => {
    ...
    return `/api/timkerja/timkerja/timkerja/${branding?.opd}/all_program_unggulan?tahun=${tahun}&bulan=${bulan}`;
  }, [isReady, tahun, bulan]);
  ...
  const hapusProgram = async (id: number) => {
    await apiFetch(`/api/timkerja/timkerja/timkerja/program_unggulan/${id}`, {
      method: "DELETE",
    }).then(resp => {
  ...
  const hapusPetugasTim = async (id: number) => {
    try {
      ...
      await apiFetch(`/api/timkerja/timkerja/petugas_tim/${id}`, {
        method: "DELETE",
      }).then(resp => {
